...
# 修改 eval_metrics 函数，支持对数转换
def eval_metrics(actual_log, pred_log):
    # 将对数尺度的值转换回原始值
    actual = np.power(10, actual_log)
...
if __name__ == "__main__":
    # 数据读取和预处理
    base_path = Path(r"C:\Users\熊萍\Desktop\固体分散体\溶出_ZJY\AUC_Cmax\AUC")
    df = pd.read_csv(base_path / "AUC_features.csv", encoding='unicode_escape')
    ...
    for model_name in models.keys():
        ...
        clipped_test_y = np.clip(all_test_y[model_name], -2, 4)  # Clip experimental values
        clipped_test_preds = np.clip(all_test_preds[model_name], -2, 4)  # Clip predicted values
        ...
        best_line_x = np.linspace(-2, 4, 100)  # Use [-2, 4] for the best-fit line
        ...
        ax_main.set_ylim(-2, 4)

        # X-axis histogram (Experimental Value)
        ax_histx = fig.add_subplot(gs[0, 0:3], sharex=ax_main)
        n, bins, patches = ax_histx.hist(clipped_test_y, bins=50, range=(-2, 4))  # Ensure histogram respects [-2, 4]
        cm = plt.get_cmap('Blues')
        norm = Normalize(vmin=min(n), vmax=max(n))
        for c, p in zip(n, patches):
            plt.setp(p, 'facecolor', cm(norm(c)))
        ax_histx.set_xlim(-2, 4)  # Explicitly set limits to match ax_main
        ax_histx.set_yticks([])
        ...
        n, bins, patches = ax_histy.hist(clipped_test_preds, bins=50, range=(-2, 4),
                                         orientation='horizontal')  # Ensure histogram respects [-2, 4]
        cm = plt.get_cmap('Blues')
        norm = Normalize(vmin=min(n), vmax=max(n))
        for c, p in zip(n, patches):
            plt.setp(p, 'facecolor', cm(norm(c)))
        ax_histy.set_ylim(-2, 4)  # Explicitly set limits to match ax_main
        ax_histy.set_xticks([])
